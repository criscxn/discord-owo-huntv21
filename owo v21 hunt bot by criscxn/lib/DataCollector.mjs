...
function listAccount(data) {
    const obj = listCheckbox("list", "Giriş yapmak lütfen bir hesap seçiniz", [
        ...new Set(Object.values(data).map(user => user.tag)), 
        {name: "Yeni Hesap Ekle (Token ile Giriş Yap)", value: 0},
        {name: "Yeni Hesap Ekle (Telefondan QR Kod ile Giriş Yap)", value: 1},
...
function music2(folder) {
    ...
    const obj = {
        type: "list",
        message: "Müzik dosyanızı seçin(GEÇMEK ÖNERİLİR)",
        choices: [
...
function captchaAPI(cache) {
    const obj = {
        ...
        choices: [
            {name: "Geç(ÖNERİLİR)", value: 0},
            {name: "TrueCaptcha (Ücretsiz)", value: 1},
            {name: "2Captcha (Ücretli)", value: 2},
            {name: "Cris Captcha Çözme API [Yakında]", disabled: true}
        ],
        loop: false
    }
    if(cache) obj.default = cache;
...
function apiUser(cache) {
    const obj = {
        type: "input",
        message: "API Kullanıcı ID'nizi Girin(GEÇMEK ÖNERİLİR)",
        validate(ans) {
...
function botPrefix(cache) {
    const obj = {
        type: "input",
        message: "[BETA] Bot Önekinizi Girin (Yalnızca Bildirim Alıcısının ve Selfbot Hesabının Erişimi Olacaktır), Atlamak İçin Boş Bırakın(GEÇMEK ÖNERİLİR)",
        validate(ans) {
            if(!ans) return true
